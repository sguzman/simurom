  ...
  for PostProcessMaterial
{
  fn vertex_shader() -> ShaderRef {
    "simurom/shaders/postprocess.wgsl"
      .into()
  }
...
pub struct SimuromEffectsPlugin;

impl Plugin for SimuromEffectsPlugin {
  fn build(
    &self,
    app: &mut App
  ) {
      ...
      &scene.0.scene.effects
    {
        ...
        list.iter().find(|e| e.id == id)
      {
        ...
        if let Some(wgsl) = &effect.wgsl
        {
          ...
          &effect.glsl
        {
          shader = simurom_assets::resolve::bevy_load::load_glsl_shader(
            &asset_server,
            &cfg.0,
            &assets_root.0,
            glsl,
        ...
        &scene.0.scene.effects
      {
          ...
          .find(|e| e.id == global_id)
        {
          ...
          if let Some(wgsl) =
            &effect.wgsl
          {
            let _ = simurom_assets::resolve::bevy_load::load_wgsl_shader(
              &asset_server,
              &cfg.0,
              &assets_root.0,
              wgsl,
            ...
            &effect.glsl
          {
            let _ = simurom_assets::resolve::bevy_load::load_glsl_shader(
              &asset_server,
              &cfg.0,
              &assets_root.0,
              glsl,
        ...
        &scene.0.scene.effects
      {
          ...
          .find(|e| e.id == id)
        {
          ...
          if let Some(wgsl) =
            &effect.wgsl
          {
            shader_handle = simurom_assets::resolve::bevy_load::load_wgsl_shader(
              &asset_server,
            ...
            &effect.glsl
          {
            shader_handle = simurom_assets::resolve::bevy_load::load_glsl_shader(
              &asset_server,
              &cfg.as_deref().unwrap().0,
              &assets_root.as_deref().unwrap().0,
              glsl,
