  ...
  match cli.command {
    ...
    | Command::TraceTimeline {
      ...
      let mut scene_file =
        load_scene(&scene)?;
      let opts =
        simurom_runtime::headless_timeline::HeadlessTimelineOptions {
          max_steps,
          max_time_secs
        };
      let res =
        simurom_runtime::headless_timeline::run_headless_timeline(
          &cfg,
          &mut scene_file,
          &opts
        )?;
